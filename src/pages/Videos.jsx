...
import { Box, Heading, Stack, VStack, HStack, Button, Text } from '@chakra-ui/react'
                ...
                <VStack width={ ["full", "xl"] } alignItems={ "stretch" } p="8" spacing={ "8" } overflowY={ "auto" }>
                    {
                        videosArr.map( ( item, index ) => (
                            <Button onClick={ () => setvideoSrc( item ) } colorScheme={ 'purple' } variant={ "ghost" }>Lecture { index + 1 }</Button>
                        ) )
                    }
                </VStack>
            </Stack>
